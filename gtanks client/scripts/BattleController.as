package
{
   ...
   import alternativa.tanks.models.sfx.shoot.hwthunder.criticalhit.ITankCriticalHitModel;
   import alternativa.tanks.models.sfx.shoot.hwthunder.criticalhit.TankCriticalHitModel;
   ...
   import scpacker.test.bots.BotModel;
   import scpacker.test.bots.IBotModel;
   ...
   public class BattleController implements INetworkListener
   {
         ...
         try
         {
            switch(data.type)
            {
               case Type.BATTLE:
                  ...
                  else if(data.args[0] == "init_bot_tank")
                  {
                     BotModel(Main.osgi.getService(IBotModel)).addBot(data.args[1]);
                  }
                  ...
                  else if(data.args[0] != "tracePing")
                  {
                     ...
                     else if(data.args[0] == "take_bonus_by")
                     {
                     ...
                     else if(data.args[0] == "kick_for_cheats")
                     {
                     ...
                     else if(data.args[0] == "start_fire_twins")
                     {
                        this.parseStartFireTwins(activeTanks[data.args[1]],data.args[2]);
                     }
                     else if(data.args[0] == "start_fire_snowman")
                     {
                     ...
                     else if(data.args[0] == "fire_ricochet")
                     {
                     ...
                     else if(data.args[0] == "change_user_team")
                     {
                     ...
                     else if(data.args[0] == "disnable_effect")
                     {
                     ...
                     else if(data.args[0] == "local_user_killed")
                     {
                     ...
                     else if(data.args[0] == "init_mines")
                     {
                     ...
                     {
                        DOMModel(Main.osgi.getService(IDOMModel)).serverSetPointScore(data.args[1],data.args[2],data.args[3]);
                     }
                     ...
                     else if(data.args[0] == "shaft_quick_shot")
                     {
                     ...
                     else if(data.args[0] == "create_critical_hit")
                     {
                        tankData = TankModel(Main.osgi.getService(ITank)).getTankData(activeTanks[data.args[1]]);
                        TankCriticalHitModel(Main.osgi.getService(ITankCriticalHitModel)).createCriticalHitEffects(tankData.tank.state.pos,tankData.tank);
                     }
                     else if(data.args[0] == "gold_spawn")
                     {
                        BattlefieldModel(Main.osgi.getService(IBattleField)).messages.addMessage(16753920,ILocaleService(Main.osgi.getService(ILocaleService)).getText(TextConst.BATTLE_GOLD_BOX));
                        soundResource = ResourceUtil.getResource(ResourceType.SOUND,"gold") as SoundResource;
                        if(soundResource != null)
                        {
                           BattlefieldModel(Main.osgi.getService(IBattleField)).soundManager.playSound(soundResource.sound,0,1);
                        }
                        else
                        {
                           Logger.log("Resource with id: sound gold!");
                        }
                     }
                     else if(data.args[0] == "move_bot")
                     {
                        this.moveBot(data.args[1]);
                     }
                     else if(data.args[0] == "init_debug_bots")
                     {
                        json = JSON.parse(data.args[1]);
                        waypoints = new Array();
                        ...
                        {
                           gr = new Array();
                           for each(waypoint in group)
                           {
                              gr.push(new Vector3(waypoint.x,waypoint.y,waypoint.z));
                           }
                           waypoints.push(gr);
                        }
                        BotModel(Main.osgi.getService(IBotModel)).initDebug(waypoints);
                     }
                     else if(data.args[0] == "init_turret_sfx_lighting")
                     {
      ...
      private function parseShotsData(js:String) : void
      {
         ...
         Logger.info(0,"parseShotsData: " + js);
         var parser:Object = JSON.parse(js);
      ...
      private function parseStopFire(param1:ClientObject, param2:String) : void
      {
         ...
         var _loc3_:IModelService = Main.osgi.getService(IModelService) as IModelService;
         if(TankModel(Main.osgi.getService(ITank)) == null || activeTanks[param1.id] as ClientObject == null)
         {
            return;
         }
         var _loc4_:TankData = TankModel(Main.osgi.getService(ITank)).getTankData(activeTanks[param1.id] as ClientObject);
         ...
         {
            case "flamethrower":
               _loc6_ = Main.osgi.getService(IFlamethrower) as FlamethrowerModel;
               _loc6_.stopFire(param1,param2);
               break;
            case "flamethrowerhw":
               _loc6_ = Main.osgi.getService(IFlamethrower) as FlamethrowerModel;
               _loc6_.stopFire(param1,param2);
               break;
            case "isida":
               isidaModel.stopWeapon(param1,param2);
               break;
            case "isidahw":
               isidaModel.stopWeapon(param1,param2);
               break;
            case "frezee":
               frezeeModel.stopFire(param1,param2);
               break;
            case "frezeeny":
               frezeeNYModel.stopFire(param1,param2);
         }
      }
      ...
      private function parseFire(user:ClientObject, js:String) : void
      {
         ...
         switch(id)
         {
            ...
            case "smokyxt":
               ...
               break;
            case "twins":
               twinsModel.hit(user,user.id,parser.shotId,new Vector3d(parser.hitPos.x,parser.hitPos.y,parser.hitPos.z),parser.victimId,1);
               break;
            case "twinsxt":
               twinsModel.hit(user,user.id,parser.shotId,new Vector3d(parser.hitPos.x,parser.hitPos.y,parser.hitPos.z),parser.victimId,1);
               break;
      ...
      {
         var botModel:BotModel = Main.osgi.getService(IBotModel) as BotModel;
         if(botModel != null)
         {
            botModel.removeBot(s);
         }
      ...
      private function moveBot(js:String) : void
      {
         var parser:Object = JSON.parse(js);
         var pos:Vector3d = new Vector3d(0,0,0);
         var orient:Vector3d = new Vector3d(0,0,0);
         ...
         pos.y = parser.position.y;
         pos.z = parser.position.z;
         orient.x = parser.orientation.x;
         orient.y = parser.orientation.y;
         orient.z = parser.orientation.z;
         line.x = parser.velocity.x;
         line.y = parser.velocity.y;
         line.z = parser.velocity.z;
         ange.x = parser.angleVelocity.x;
         ange.y = parser.angleVelocity.y;
         ange.z = parser.angleVelocity.z;
         if(activeTanks[parser.botName] != null)
         {
            TankModel(Main.osgi.getService(ITank)).move(activeTanks[parser.botName] as ClientObject,pos,orient,line,ange,parser.turretAngle,parser.ctrl,true);
         }
      }
      ...
      private function initBattle(js:String) : void
      {
         ...
         this.battle.bm.initObject(client,null,soundsParams,json.kick_period_ms,json.map_id,json.invisible_time,json.skybox_id,json.spectator,gameMode,resources);
      }
