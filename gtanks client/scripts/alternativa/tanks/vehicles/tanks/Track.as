   ...
   public class Track
   {
      ...
      public function addForces(dt:Number, throttle:Number, maxSpeed:Number, slipTerm:int, weight:Number, data:SuspensionData, brake:Boolean) : void
      {
         ...
         if(this.raysNum & 1 == 0)
         {
            for(i = 0; i < this.raysNum; i++)
            {
               SuspensionRay(this.rays[i]).addForce(dt,throttle,maxSpeed,i <= mid ? int(slipTerm) : int(-slipTerm),springCoeff,data,brake);
            }
         }
         else
         {
            for(i = 0; i < this.raysNum; i++)
            {
               SuspensionRay(this.rays[i]).addForce(dt,throttle,maxSpeed,i < mid ? int(slipTerm) : (i > mid ? int(-slipTerm) : int(0)),springCoeff,data,brake);
            }
         }
      }
      ...
      public function setAnimationSpeed(param1:Number, param2:Number) : void
      {
         ...
         if(this.animationSpeed < param1)
         {
            ...
            this.animationSpeed = _loc3_ > param1 ? Number(Number(param1)) : Number(Number(_loc3_));
         }
