package alternativa.tanks.model
{
   ...
   public class ChatModel extends ChatModelBase implements IChatModelBase, IObjectLoadListener
   {
      ...
      public function showMessages(clientObject:ClientObject, messages:Array) : void
      {
         ...
         {
            message = messages[i];
            this.chatPanel.addMessage(message.sourceUser.uid,message.sourceUser.rankIndex,this.replaceBattleLink(message.text),message.targetUser != null ? int(message.targetUser.rankIndex) : int(0),message.targetUser != null ? message.targetUser.uid : "",message.system,message.sysCollor);
         }
         if(i > 25)
         {
      ...
      private function checkBuffer() : void
      {
         ...
         while(this.messageBuffer.length > 0 && !stop)
         {
            ...
            if(!stop)
            {
               ...
               this.chatPanel.addMessage(message.sourceUser.uid,message.sourceUser.rankIndex,message.text,message.targetUser != null ? int(message.targetUser.rankIndex) : int(0),message.targetUser != null ? message.targetUser.uid : "");
