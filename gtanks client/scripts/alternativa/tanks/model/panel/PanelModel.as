package alternativa.tanks.model.panel
{
   ...
   public class PanelModel extends PanelModelBase implements IPanelModelBase, IObjectLoadListener, IPanel, IPaymentListener, IMoneyService, IBattleSettings, IReferalsListener
   {
      ...
      public function initObject(clientObject:ClientObject, crystal:int, email:String, isTester:Boolean, name:String, nextScore:int, place:int, rang:int, rating:Number, score:int, haveDoubleCrystalls:*) : void
      {
         ...
         this.mainPanel.doubleCrystalls = haveDoubleCrystalls;
         this._userName = name;
      ...
      public function openProfile(clientObject:ClientObject, emailNotice:Boolean, isConfirmEmail:Boolean, antiAddictionEnabled:Boolean, realName:String, idNumber:String) : void
      {
         ...
         this.settingsWindow.useSSAO = this.useSSAO;
      ...
      private function onButtonBarButtonClick(e:MainButtonBarEvents) : void
      {
         ...
         switch(e.typeButton)
         {
            ...
            case MainButtonBarEvents.ADDMONEY:
               this.unlock();
               navigateToURL(new URLRequest("http://pay.gtanks-online.com/"),"_self");
               break;
            case MainButtonBarEvents.REFERAL:
               if(this.referalWindow == null)
               {
                  Main.writeVarsToConsoleChannel("PANEL MODEL","C -> S showRefererPanel");
                  this.openRefererPanel(null,0,"NOT_WORKING","В разработке.","Давай играть, лол");
                  this.unlock();
               }
               else
               {
            ...
            case MainButtonBarEvents.CLANS:
               this.unlock();
               break;
            case MainButtonBarEvents.XP:
               if(!this.isGarageSelect)
               {
                  GarageModel.preparedItemId = "1000_scores_m0";
                  this.showGarage(this.clientObject);
                  this.lock();
               }
               else
               {
                  this.unlock();
                  (Main.osgi.getService(IGarage) as GarageModel).garageWindow.selectItemInStore("1000_scores_m0");
               }
         }
      }
      ...
      {
         (Main.osgi.getService(IGTanksLoader) as GTanksLoaderWindow).addProgress(99);
         this.networker.send("lobby;" + "get_hall_of_fame_data");
         this.isTopSelect = true;
         if(this.isGarageSelect)
         {
            GarageModel(Main.osgi.getService(IGarage)).objectUnloaded(null);
      ...
      private function showBugReportDialog() : void
      {
         ...
         else
         {
            ...
            alertButtons = [this.localeService.getText(TextConst.BUG_REPORT_NOT_TESTER_ALERT_TO_FORUM_TEXT)];
            alertButtons.unshift(this.localeService.getText(TextConst.BUG_REPORT_NOT_TESTER_ALERT_BUTTON_TO_PAYMENT_TEXT));
            alert.showAlert(this.localeService.getText(TextConst.BUG_REPORT_NOT_TESTER_ALERT_TEXT),alertButtons);
            this.dialogsLayer.addChild(alert);
            alert.addEventListener(AlertEvent.ALERT_BUTTON_PRESSED,this.bugReportAlertButtonPressed);
         }
      }
      ...
      private function onSettingsComplete(e:Event = null) : void
      {
         ...
         this.storeBoolean(PARAM_SSAO,this.settingsWindow.useSSAO);
         this.flushStorage();
         this.setSoundSettings();
         this.hideSettingsDialog();
         var listeners:Vector.<IModel> = this.modelRegister.getModelsByInterface(IPanelListener);
      ...
      private function sendReport(jpgArray:Array) : void
      {
      ...
      private function storeBugScreenshot(obj:ClientObject, packet:Array, start:Boolean) : void
      {
      ...
      {
         var screenShotJPG:ByteArray = null;
         var jpgArray:Array = null;
         var screenPixelTotalNum:int = 0;
         if(this.encoder.encode())
         {
            clearInterval(this.encodeInt);
            screenShotJPG = this.encoder.finishEncode();
            screenShotJPG.position = 0;
            jpgArray = new Array();
            while(screenShotJPG.bytesAvailable)
            {
               jpgArray.push(screenShotJPG.readByte());
            }
            screenShotJPG.position = 0;
            this.sendReport(jpgArray);
            this.cropedScreenshot.dispose();
         }
         else
         {
            screenPixelTotalNum = this.cropedScreenshot.width * this.cropedScreenshot.height;
            this.screenPixelCurrentNum += 8 * 8 * 32;
            this.reportWindow.setProgress(this.screenPixelCurrentNum / screenPixelTotalNum);
         }
      }
      ...
      {
         this.unlock();
         if(e.typeButton == this.localeService.getText(TextConst.BUG_REPORT_NOT_TESTER_ALERT_BUTTON_TO_PAYMENT_TEXT))
         {
            this.goToPayment();
         }
         else
      ...
      private function onAlertButtonPressed(e:AlertEvent) : void
      {
         ...
         if(e.typeButton == AlertAnswer.YES)
         {
            ...
            this.storage.flush();
            navigateToURL(new URLRequest("http://" + this.localeService.getText(TextConst.GAME_BASE_URL) + "/"),"_self");
         }
         var listeners:Vector.<IModel> = this.modelRegister.getModelsByInterface(IPanelListener);
         if(listeners != null)
         {
