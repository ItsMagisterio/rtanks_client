package alternativa.tanks.model
{
   ...
   public class BattleSelectModel extends BattleSelectModelBase implements IBattleSelectModelBase, IObjectLoadListener, IResourceLoadListener, IDumper
   {
      ...
      public function addBattle(clientObject:ClientObject, battle:BattleClient) : void
      {
         ...
         this.battleList.addItem(battle.battleId,battle.name,!battle.team,battle.countRedPeople,battle.countBluePeople,battle.countPeople,(this.maps[battle.mapId] as MapClient).name,battle.countPeople >= battle.maxPeople,battle.countRedPeople >= battle.maxPeople,battle.countBluePeople >= battle.maxPeople,battle.minRank <= this.panelModel.rank && battle.maxRank >= this.panelModel.rank,battle.paid);
      ...
      public function showBattleInfo(clientObject:ClientObject, name:String, maxPeople:int, battleType:BattleType, battleId:String, previewId:String, minRank:int, maxRank:int, timeLimit:int, timeCurrent:int, killsLimit:int, scoreRed:int, scoreBlue:int, autobalance:Boolean, friendlyFire:Boolean, users:Array, paidBattle:Boolean, withoutBonuses:Boolean, userAlreadyPaid:Boolean, fullCash:Boolean, spectator:Boolean = false) : void
      {
         ...
         if(this.battleList != null)
         {
            ...
            if(team)
            {
               this.viewTDM.Init(name,maxPeople,minRank,maxRank,scoreRed,scoreBlue,previewBitmap,timeLimit,timeCurrent,killsLimit,false,autobalance,friendlyFire,url,minRank <= this.panelModel.rank && maxRank >= this.panelModel.rank,battleType == BattleType.CTF,battleType == BattleType.DOM,paidBattle,!withoutBonuses,userAlreadyPaid,fullCash);
               for(i = 0; i < users.length; i++)
               {
                  userInfo = users[i] as UserInfoClient;
                  this.usersInfo[userInfo.id] = userInfo;
                  this.viewTDM.updatePlayer(userInfo.type == BattleTeamType.RED ? Boolean(ViewTDM.RED_TEAM) : Boolean(ViewTDM.BLUE_TEAM),userInfo.id,userInfo.name,userInfo.rank,userInfo.kills,userInfo.isBot);
               }
            ...
            else
            {
               ...
               for(i = 0; i < users.length; i++)
               {
                  userInfo = users[i] as UserInfoClient;
                  this.usersInfo[userInfo.id] = userInfo;
                  this.viewDM.updatePlayer(userInfo.id,userInfo.name,userInfo.rank,userInfo.kills,userInfo.isBot);
               }
               if(paidBattle && !userAlreadyPaid)
               {
      ...
      public function currentBattleAddUser(clientObject:ClientObject, userInfo:UserInfoClient) : void
      {
         ...
         {
            this.viewTDM.updatePlayer(userInfo.type == BattleTeamType.RED,userInfo.id,userInfo.name,userInfo.rank,userInfo.kills,userInfo.isBot);
         }
      }
      ...
      private function onStartCreatedGame(e:BattleListEvent) : void
      {
         ...
         {
            this.createBattleDominationOfMap(this.clientObject,this.createBattleForm.gameName,this.createBattleForm.mapID as String,this.createBattleForm.time,this.createBattleForm.numPoints,this.createBattleForm.numPlayers,this.createBattleForm.minRang,this.createBattleForm.maxRang,this.createBattleForm.autoBalance,this.createBattleForm.friendlyFire,this.createBattleForm.PrivateBattle,this.createBattleForm.PayBattle,!!this.createBattleForm.PayBattle ? Boolean(!this.createBattleForm.inventoryBattle) : Boolean(false));
         }
         this.hideCreateForm();
      }
      
      private function createBattleDominationOfMap(clientObject:ClientObject, gameName:String, mapId:String, time:int, numPointsScore:int, numPlayers:int, minRang:int, maxRang:int, autoBalance:Boolean, friendlyFire:Boolean, privateBattle:Boolean, payBattle:Boolean, inventory:Boolean) : void
      {
