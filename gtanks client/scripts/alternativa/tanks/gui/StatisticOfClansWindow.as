package alternativa.tanks.gui
{
   ...
   public class StatisticOfClansWindow extends Sprite
   {
      
      public static var rowHeight:int;
      ...
      public function StatisticOfClansWindow(rowHeight:int)
      {
         this.overlay = new Sprite();
         super();
         StatisticOfClansWindow.rowHeight = rowHeight;
         ...
         this.list = new StatList(rowHeight);
         addChild(this.list);
         this.list.x = this.windowMargin + this.spaceModule + 1;
         this.list.y = this.inner.y + this.spaceModule;
      }
      ...
      public function setData(startPos:int, data:Array) : void
      {
         ...
         {
            info = data[i] as User;
            this.list.addItem(info.place + 1,info.rank,info.callsign,info.achiviments,info.score,info.kills,info.deaths,info.ratio,info.wealth,info.rating);
         }
      }
