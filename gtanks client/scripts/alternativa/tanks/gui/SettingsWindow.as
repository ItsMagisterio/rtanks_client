package alternativa.tanks.gui
{
   ...
   public class SettingsWindow extends Sprite
   {
      ...
      private var _ssao:TankCheckBox;
      ...
      public function SettingsWindow(mail:String, emailConfirmed:Boolean, antiAddictionEnabled:Boolean, realName:String, idNumber:String)
      {
         ...
         this.windowSize = new Point(444,540 + (!!antiAddictionEnabled ? 110 : 50) + (!this.isPasswordChangeDisabled ? 30 : 0));
         ...
         this._ssao = this.createCheckBox("SSAO");
         this._ssao.x = this._useFog.x;
         this._ssao.y = this._tracksAnimation.y + this._tracksAnimation.height + 6;
         addChild(this._ssao);
         this.performanceInner.height = this._shadows.y + margin * 2;
         ...
         this.keyBinder = new KeyboardBinder(Main.stage);
         this.keyBinder.bind("ESCAPE",this.onEscClick);
         this.keyBinder.bind("ENTER",this.onEnterClick);
      ...
      {
         return this._ssao.checked;
      }
      
      public function set useSSAO(v:Boolean) : *
      {
         this._ssao.checked = v;
      }
      ...
      private function onEnterClick(isKeyDown:Boolean) : void
      {
         if(!isKeyDown)
         {
            return;
      ...
      private function onEscClick(isKeyDown:Boolean) : void
      {
         if(!isKeyDown)
         {
            return;
