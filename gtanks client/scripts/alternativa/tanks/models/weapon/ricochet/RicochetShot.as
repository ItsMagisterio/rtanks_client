package alternativa.tanks.models.weapon.ricochet
{
   ...
   import alternativa.tanks.models.sfx.ExternalObject3DPositionProvider;
   ...
   public class RicochetShot extends PooledObject implements IGraphicEffect, IRayCollisionPredicate
   {
      ...
      public function init(startPos:Vector3, direction:Vector3, shooterData:TankData, ricochetData:RicochetData, sfxData:RicochetSFXData, collisionDetector:ICollisionDetector, weakeningModel:IWeaponWeakeningModel, listener:IRicochetShotListener, battlefield:IBattleField) : void
      {
         ...
         this.lightEffect = AnimatedLightEffect(objectPool.getObject(AnimatedLightEffect));
         this.lightEffectPositionProvider = ExternalObject3DPositionProvider(objectPool.getObject(ExternalObject3DPositionProvider));
         this.lightEffect.init(this.lightEffectPositionProvider,LightDataManager.getLightDataShot(shooterData.turret.id),AnimatedLightEffect.DEFAULT_MAX_DISTANCE,true);
         battlefield.addGraphicEffect(this.lightEffect);
      }
      ...
      public function play(millis:int, camera:GameCamera) : Boolean
      {
         ...
         var _loc2_:AnimatedLightEffect = null;
         ...
         while(frameDistance > 0)
         {
            ...
            if(hitTime > -1)
            {
               ...
               _loc2_ = AnimatedLightEffect(objectPool.getObject(AnimatedLightEffect));
               _loc3_ = StaticObject3DPositionProvider(objectPool.getObject(StaticObject3DPositionProvider));
               _loc3_.init(this.currPos,50);
               _loc2_.init(_loc3_,LightDataManager.getLightDataExplosion(this.shooterData.turret.id));
               this.battlefield.addGraphicEffect(_loc2_);
            }
            else
            {
         ...
         this.lightEffectPositionProvider.setPosition(this.currPos);
         return true;
      }
