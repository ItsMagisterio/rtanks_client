package forms
{
   ...
   public class LobbyChat extends Sprite
   {
      ...
      {
         this.sharpLinks = ["vk|ВКонтакте|https://vk.com/gtanksonline".split("|"),"rules|Правила|https://forum.gtanksonline.com/index.php?/topic/1427-%D0%BF%D1%80%D0%B0%D0%B2%D0%B8%D0%BB%D0%B0-%D0%B8%D0%B3%D1%80%D1%8B/".split("|"),"facebook|Facebook|https://www.facebook.com/GTanksOnline.en".split("|"),"ranks|Звания|http://link.com".split("|"),"forum|Форум|https://forum.gtanksonline.com/".split("|"),"wiki|Энциклопедия игры|http://wiki.gtanksonline.com/index.php/%D0%AD%D0%BD%D1%86%D0%B8%D0%BA%D0%BB%D0%BE%D0%BF%D0%B5%D0%B4%D0%B8%D1%8F_%D0%B8%D0%B3%D1%80%D1%8B_%C2%ABGTanks%C2%BB".split("|")];
         trace(this.localeService.getText(TextConst.CHAT_SHARP_FAQ));
      ...
      public function addMessage(username:String, rang:int, text:String, rangTo:int = 0, nameTo:String = "", system:Boolean = false, _systemColor:uint = 8454016) : void
      {
         ...
         {
            messageText = line;
            if(messageText.indexOf(" !proof!(") >= 0)
            {
               user = null;
               if(messageText.substring(messageText.length - 1) != ")")
               {
                  user = messageText.split(" !proof!(")[1].split(")")[1];
               }
               proofId = messageText.split(" !proof!(")[1].replace(")" + user,"");
               proofId = proofId.replace(" !proof!(" + proofId + ")" + user,"");
               replaced = " !proof!(" + proofId + ")" + user;
               text = text.replace(replaced," <u><a href=\"http://185.97.255.32:8080/moderation-tools/?proofs=" + proofId + "&user=" + user + "\" target=\"_blank\">(" + proofId + ")</a></u>");
         ...
         if(!blocked(username))
         {
            this.output.addLine(rang,username,text,rangTo,nameTo,system,this.htmlFlag,_systemColor);
            if(this.addressed)
            {
               this.output.selectUser(this._nameTo);
            }
