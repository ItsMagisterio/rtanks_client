package forms.battlelist
{
   ...
   public class ViewTDM extends Sprite
   {
      ...
      private function ConfigUI(e:Event) : void
      {
         ...
         this.mainBackground.header = TankWindowHeader.BATTLE_INFO;
      ...
      public function Init(gameName:String, sizeTeam:int, minRang:int, maxRang:int, scoreRed:int, scoreBlue:int, img:BitmapData = null, lifeTime:Number = 0, currentTime:int = 0, killsLimit:int = 0, inviteOnly:Boolean = false, autoBalance:Boolean = false, friendlyFire:Boolean = false, url:String = "", available:Boolean = true, ctf:Boolean = false, dom:Boolean = false, payBattle:Boolean = false, inventoryOn:Boolean = false, userAlreadyPaid:Boolean = false, fullCash:Boolean = false) : void
      {
         ...
         this.info.setUp(this._gameName,this._minRang,this._maxRang,this._killsLimit,this._lifeTime,this._currentTime,this._img,this._inviteOnly,this._autoBalance,this._friendlyFire,url,this._ctf,dom,payBattle,inventoryOn);
         ...
         this.fightButtonBlue.cost = this._haveSubscribe || !payBattle || this._userAlreadyPaid ? int(0) : int(5);
         this.fightButtonRed.cost = this._haveSubscribe || !payBattle || this._userAlreadyPaid ? int(0) : int(5);
      ...
      public function updatePlayer(team:Boolean = true, id:Object = null, name:String = "", rang:int = 0, kills:int = 0, bot:Boolean = false) : void
      {
         var index:int = 0;
         var i:int = 0;
         ...
         item.playerName = !!bot ? "[BOT] " + name : name;
         ...
         for(i = 0; i < this.dpRed.length; i++)
         {
            this.countRed += this.dpRed.getItemAt(i).id != null ? 1 : 0;
         }
         for(i = 0; i < this.dpBlue.length; i++)
         {
            this.countBlue += this.dpBlue.getItemAt(i).id != null ? 1 : 0;
         }
         this.dpRed.sortOn(["kills","rang"],[Array.DESCENDING | Array.NUMERIC,Array.DESCENDING | Array.NUMERIC]);
         this.dpBlue.sortOn(["kills","rang"],[Array.DESCENDING | Array.NUMERIC,Array.DESCENDING | Array.NUMERIC]);
         ...
         this.fightButtonRed.enable = (!!this._available ? Boolean(this.countRed <= this.countBlue || !this._autoBalance) : Boolean(false)) && this.countRed < this._sizeTeam && (this._fullCash || !this._payBattle || this._userAlreadyPaid || this._haveSubscribe);
         this.fightButtonBlue.enable = (!!this._available ? Boolean(this.countBlue <= this.countRed || !this._autoBalance) : Boolean(false)) && this.countBlue < this._sizeTeam && (this._fullCash || !this._payBattle || this._userAlreadyPaid || this._haveSubscribe);
