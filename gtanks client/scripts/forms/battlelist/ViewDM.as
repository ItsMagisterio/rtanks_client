package forms.battlelist
{
   ...
   public class ViewDM extends Sprite
   {
      ...
      public function Init(gameName:String, sizeTeam:int, minRang:int, maxRang:int, img:BitmapData = null, lifeTime:Number = 0, currentTime:int = 0, killsLimit:int = 0, inviteOnly:Boolean = false, url:String = "", available:Boolean = true, payBattle:Boolean = false, inventoryOn:Boolean = false, userAlreadyPaid:Boolean = false, fullCash:Boolean = false) : void
      {
         ...
         this.info.setUp(this._gameName,this._minRang,this._maxRang,this._killsLimit,this._lifeTime,this._currentTime,this._img,this._inviteOnly,false,false,url,false,false,payBattle,inventoryOn);
         ...
         this.fightButton.cost = this._haveSubscribe || !payBattle || this._userAlreadyPaid ? int(0) : int(5);
         this.fillTeam();
         this.onResize(null);
      }
      ...
      public function updatePlayer(id:Object = null, name:String = "", rang:int = 0, kills:int = 0, bot:Boolean = false) : void
      {
         var index:int = 0;
